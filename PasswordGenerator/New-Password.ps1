...
[int]$Length = Get-VstsInput -Name length
[bool]$isPlainText = Get-VstsInput -Name isPlainText -AsBool
...
if ($isPlainText) {
    Write-Host "##vso[task.setvariable variable=$ResultName]$GeneratedPassword"
}
else {
    Write-Host "##vso[task.setvariable variable=$ResultName;issecret=true]$GeneratedPassword"
}